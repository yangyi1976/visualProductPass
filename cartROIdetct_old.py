...
class CartROIdetector(object ):
    ...
    def preProcessImg(self,img_src):
        ...
        img_src=cv.Canny(img_src1,24,128)
        cv.imshow('Original cany Filter image', img_src)
        cv.waitKey(0)
        # 将图像转化成标准大小
        h,w=img_src.shape[:2]
        # if w>=1920:
        ...
        kernel = cv.getStructuringElement(cv.MORPH_RECT, (int(7), int(7)))

        dilate = cv.dilate(img_src, kernel, iterations=3)
